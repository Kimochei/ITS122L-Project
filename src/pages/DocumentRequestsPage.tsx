...
// Define the structure of a request for tracking
...
const DocumentRequestsPage = () => {
  ...
  // --- FIX 1: Add all required state variables for the detailed form ---
  const [fullName, setFullName] = useState('');
  const [age, setAge] = useState('');
  const [dateOfBirth, setDateOfBirth] = useState('');
  const [address, setAddress] = useState('');
  const [documentType, setDocumentType] = useState(''); // Default to empty to force selection
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    // --- FIX 2: Construct the data object to match your FastAPI backend schema ---
    const requestData = {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Submission failed. Please check all fields.');
      }
  ...
  // Handle form submission for tracking a request
  ...
  const renderContent = () => {
    if (view === 'create') {
      // This 'create' view now renders the full, detailed form.
      return (
      ...
      <div className={styles.formContainer}>
        ...
        {trackedRequest && (
            <div className={styles.trackedResult}>
                ...
                <p><strong>Status:</strong> <span className={styles.status}>{trackedRequest.status}</span></p>
            </div>
        )}
      </div>
    </div>
